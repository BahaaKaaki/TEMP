...
107. **NEOM Authority client profile**: Selectable `neom` profile from NAFB5 deck (`22042026- NEOM Authority_NAFB5_vF.pptx`), slide 46 + master geometry (13.333x7.5in, 72px/in). Title 29,29,902x44 (20pt ALL CAPS, max 2 lines); subtitle 29,77,902x25 (18pt ALL CAPS optional); frame 29,110,902x390. Colors `#13100D` / `#EBC03F` / `#FFFFFF`. Footer branding `NEOM AUTHORITY ACTIVATION`; logo chrome bottom-left (70x19 at 35,505) from `backend/assets/client-templates/neom/logo.png`; bundled master at `backend/assets/client-templates/neom/default-master.pptx`.
108. **NEOM preview/export chrome parity**: Shared `slideChromeUtils.js` injects footer branding, page numbers, and bottom-left logo (never top-left). NEOM chrome rules live in `slides.css` (footer logo ~35,507 90x24); PDF/HTML export uses the same prep path as `SlidePreview`. PPTX export prefers **Content 1 Col, White** layout and strips header-band pictures from generated slide XML. Title/subtitle overlap fixed via separated y positions; empty footer spans auto-fill activation text.
