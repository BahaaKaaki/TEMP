...
107. **NEOM Authority client profile**: Selectable `neom` profile from NAFB5 deck (`22042026- NEOM Authority_NAFB5_vF.pptx`), slide 46 + master geometry (13.333x7.5in, 72px/in). Title 29,29,902x44 (20pt ALL CAPS bold, max 2 lines); subtitle 29,77,902x25 (18pt ALL CAPS regular, optional); frame 29,110,902x390. Colors `#13100D` / `#EBC03F` / `#FFFFFF`. Step/index badges: `#13100D` fill + `#FFFFFF` text (`normalizeNeomContrastSlide` rewrites Strategy& grey fills). Footer: bottom-left logo + page number only; preview chrome uses `logo-icon.jpeg` + `logo.png`; PPTX export strips master `NEOM AUTHORITY ACTIVATION` shapes; bundled master at `backend/assets/client-templates/neom/default-master.pptx`.
108. **NEOM preview/export chrome parity**: Shared `slideChromeUtils.js` injects footer branding, page numbers, and dual footer chrome (master icon `logo-icon.jpeg` at ~5,509 27x26 + wordmark `logo.png` at ~35,517 45x12). HTML preview adds NAFB5 gold footer rule (`#EBC03F` line ~80–893 at y517, activation label centered below at y527, page number at x897 with ~37px right margin). PDF/HTML export uses the same prep path as `SlidePreview`. PPTX export prefers **Content 1 Col, White** layout and strips header-band pictures from generated slide XML.
