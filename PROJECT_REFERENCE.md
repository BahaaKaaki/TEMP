...
108. **NEOM preview/export chrome parity**: Shared `slideChromeUtils.js` injects footer branding, page numbers, and bottom-left logo (never top-left). NEOM chrome rules live in `slides.css`; PDF/HTML export uses the same prep path as `SlidePreview`. Title/subtitle overlap fixed via separated y positions; empty footer spans auto-fill activation text.
