...
107. **NEOM Authority client profile**: Selectable `neom` profile from NAFB5 deck (`22042026- NEOM Authority_NAFB5_vF.pptx`), slide 46 + master geometry (13.333x7.5in, 72px/in). Title 29,29,902x44 (20pt ALL CAPS, max 2 lines); subtitle 29,77,902x25 (18pt ALL CAPS optional); frame 29,110,902x390. Colors `#13100D` / `#EBC03F` / `#FFFFFF`. Footer branding `NEOM AUTHORITY ACTIVATION`; preview chrome uses `logo-icon.jpeg` + `logo.png` at master positions; bundled master at `backend/assets/client-templates/neom/default-master.pptx`.
