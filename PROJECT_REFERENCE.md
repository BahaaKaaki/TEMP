...
97. **DGE chrome vs native template**: Slide title and subtitle default to black #000000 with Noto Sans Bold 30px / Regular 12px (per `20260512_DGE_PPT Presentation Template_Edwin.pptx` slide 7 XML); brand blues remain for body/panels. Default `.frame` widened to 894x366 at y=127 so timelines and wide layouts match usable canvas; `DGE_LAYOUT_CONTRACT.timeline` keeps exact four-card positions from that slide.
