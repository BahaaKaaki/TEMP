...
```javascript
{
  ...
  settings: {
    ...
    model: 'pwc:bedrock.anthropic.claude-opus-4-8',  // Premium generation (Claude Opus 4.8 on Bedrock)
    ...
    pptxModel: 'pwc:bedrock.anthropic.claude-opus-4-8', // PPTX export code generation
...
69. **Per-step evidence search model**: SmartAction per-step web searches now use `settings.evidenceSearchModel` with a code default of `openai.gpt-5.4-mini`, keeping classifier/search defaults lightweight while improving factual enrichment quality for slide-specific evidence. Dependent steps now preserve canonical entities from `contextFromStep` and use search for enrichment fields instead of broad rediscovery.
...
95. **DGE client template profile**: Department of Government Enablement (Abu Dhabi) profile in `clientDesignProfiles.js` with blues (#063360 primary panels, **#203864** top-tab/tracker chrome from client reference decks, #0A4D73 structural, #7DA1C4 borders/light panels), Noto Sans stack (SemiBold/Medium/Regular/Bold TTFs in `backend/assets/fonts/dge-noto/`), bundled `backend/assets/client-templates/dge/default-master.pptx` and `logo.png`, backend routes `?profileId=dge` and `/api/assets/fonts/dge-noto/`, canvas chrome and `slides.css` rules for `data-client-profile="dge"`, and PPTX hints (`pptxService.js`) aligned to the DGE shell.
...
100. **Visual Uplift (image-content)**: `upliftSlideWithImage()` keeps title/subtitle/footer, replaces `.frame` with a `data:image` `<img class="frame-image">`, and generates via code-managed `settings.imageModel` (`openai.global.gpt-image-2` by default). Visual Uplift is off by default; debug Settings exposes `visualUpliftEnabled` for the AI-panel button and **Visual direction** popover. Prompt override `visualUplift.system` remains in debug Settings. Frame image prompts share `buildFrameImagePaletteGuidance()` + deck/profile theme tokens.
...
104. **SE client template profile**: Saudi Electricity (SE) selectable profile in `clientDesignProfiles.js` with brand-guideline blues (#001F5E, #0080FF, #32C2FF), subtitle accent #00FF86, SE Medium 32px titles / SE bold 18px subtitles, title band at y=32 (below top accent bar), layout bands from `SE Slide Repository - New Style.pptx`, bundled `backend/assets/client-templates/se/default-master.pptx` and `logo.png`, SE TTF weights under `backend/assets/fonts/se/`, backend routes `?profileId=se` and `/api/assets/fonts/se/`, canvas chrome in `slides.css` / `slidePreviewMeasureCss.js`, and PPTX font hints in `pptxService.js`.
105. **Post-generation layout polish (V1)**: After `generateSlides` parses HTML, optional fast-model pass (`slideLayoutPolish.js`, default on via `settings.slideLayoutPolish`) runs one LLM call per slide before the deck is shown. Fixes frame-only layout issues (whitespace, grid alignment, overlaps, overflow, aesthetics) using `fastModel` (default Gemini 3.5 Flash). Title, subtitle, footer, and section tracker chrome are restored from the pre-polish HTML. System prompt always appends active client `layoutContract` frame coordinates via `buildLayoutPolishGeometryBlock()` (STC 931x340, SE 890x353, etc.). Toggle in Generation settings; editable overrides `layoutPolish.system` and `layoutPolish.user` in Settings → Prompts → Layout Polish.
106. **Dense frame layout auto-normalize**: `slideFrameLayoutNormalize.js` detects portfolio grids (`.portfolio` / colHead+rowHead) and roadmap phase columns (`.roadmap` / `.phases`+`.timeline`) and appends idempotent CSS patches so flex/grid children shrink inside `div.frame` (`overflow:hidden`). Wired through `parseGeneratedSlides`, layout polish merge, `SlideContext` add/update/import, and load-time migration. STC title/subtitle/frame geometry also hardcoded in `slides.css` as fallback when theme CSS is omitted from preview export.
107. **NEOM Authority client profile**: Selectable `neom` profile from NAFB5 deck (`22042026- NEOM Authority_NAFB5_vF.pptx`), slide 46 + master geometry (13.333x7.5in, 72px/in). Title 29,29,902x44 (20pt ALL CAPS bold, max 2 lines); subtitle 29,77,902x25 (18pt ALL CAPS regular, optional); frame 29,110,902x390. Colors `#13100D` / `#EBC03F` / `#FFFFFF`. Contrast tokens: `var(--on-accent)` on yellow fills only; `var(--on-neutral-fill)` / `#FFFFFF` on `#13100D` / `var(--neutral-fill)` (hub, capNum, step badges). `normalizeNeomContrastSlide` rewrites generated CSS and appends `.hub`/`.capNum` patches. Footer: bottom-left logo + non-editable chrome page number (`client-chrome-neom-pagenum`); no section tracker pills in canvas. PPTX merge passes `profile` into `applyTemplateToGenerated`, sanitizes all masters/layouts + `p14:sectionLst`, sets `showMasterSp="0"`, injects bottom logo/icon + single page number (skips `addFooter` during generation). Bundled master sanitized via `backend/scripts/sanitize-neom-master-pptx.py`.
108. **NEOM preview/export chrome parity**: Shared `slideChromeUtils.js` injects dual footer chrome (icon + wordmark), gold footer rule in `slides.css`, and page number at master coords via `data-no-edit` span (editable footer last span hidden). PDF/HTML export uses `prepareSlideHtmlForRender` like `SlidePreview`. PPTX export skips `addSectionTracker` for NEOM; prefers **Content 1 Col, White** layout.
109. **Client template picker UI**: Header `ClientDesignProfileSwitch` uses searchable scrollable panel with per-profile color swatches (page/heading/accent) and descriptions; Strategy& stays pinned first.
110. **Opus 4.8 defaults (premium/report/PPTX)**: Code-managed defaults in `SlideContext.jsx` (`settings.model`, `settings.reportModel`, `settings.pptxModel`) and `pptxService.js` (`DEFAULT_PPTX_MODEL`) use `pwc:bedrock.anthropic.claude-opus-4-8`; temperature omission regex covers Opus 4.7 and 4.8.
111. **Remat client design profile**: Selectable `remat` profile from `Remat PowerPoint EN.pptx` (green SST Arabic branding, white layout with footer logo band). Bundled slim master + logo under `backend/assets/client-templates/remat/`, SST Arabic fonts under `backend/assets/fonts/sst-arabic/` with `/api/assets/fonts/sst-arabic/:file` route; preview CSS, `clientDesignProfiles.js` contracts, and PPTX hints in `pptxService.js`.
112. **ECA CP client design profile**: Selectable `eca` profile from `20260614_ECA CP_template 2_for Edwin AI.pptx` (white `1_Title Only - White` layout, Tahoma, teal `#41A4A5` titles, no subtitle, per-slide section tracker at y=4). Bundled master rebuilt via `backend/scripts/build-eca-cp-master-pptx.py` (valid Content_Types, cleaned rels, no think-cell); chart palette includes orange `#D95022`, green `#7FBC80`, purple `#5E417B`, cyan `#53B0C5`.
113. **TDREDC client template profile**: Selectable `tdredc` profile from `20260628_TDREDC Template.pptx` (Tower District Real Estate Development Company ExCom template; PIF-adjacent Fund typography and green/gold palette on a 13.333 x 7.5 in shell). Bundled `backend/assets/client-templates/tdredc/default-master.pptx`, full lockup `logo.png` (from template `image2.svg` green-on-white lockup), and `logo.svg`; reuses Fund fonts from `backend/assets/fonts/pif-fund/`; backend route `?profileId=tdredc`; preview logo chrome at 33, 18, 200, 70 px with title band at 280, 45; workstream pillar tokens Rise `#46121D`, NoRA `#9D7539`, Corporate `#2C2C2C`; preview CSS, `clientDesignProfiles.js` contracts, and PPTX hints in `pptxService.js`.
