...
108. **NEOM preview/export chrome parity**: Shared `slideChromeUtils.js` injects footer branding, page numbers, and dual footer chrome (master icon `logo-icon.jpeg` at ~5,509 27x26 + wordmark `logo.png` at ~35,517 45x12). PDF/HTML export uses the same prep path as `SlidePreview`. PPTX export prefers **Content 1 Col, White** layout and strips header-band pictures from generated slide XML. Title/subtitle overlap fixed via separated y positions; empty footer spans auto-fill activation text.
