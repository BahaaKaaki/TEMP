...
| Image | `openai.gpt-image-1.5` (regional; ~8–12s). `openai.global.gpt-image-2` needs PwC geography enablement — proxy auto-falls back to 1.5 on 401. **Visual Uplift** always uses `vertex_ai.gemini-3-pro-image-preview` (~20s). Chat/Quick Fix edits on image slides regenerate the frame image (`editRasterImageSlide`) on the same pipeline as creation (uplift vs Settings image model); text HTML edit is fallback only. Uplifted/image-content slides export to PPTX by embedding the raster image (not re-translating HTML). |
