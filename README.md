...
- Client template profiles can switch generation away from the default Strategy& look; STC, PIF LDC, and DGE ship as full profiles with semantic theme tokens, layout CSS variables, prompt-section overrides, bundled masters/assets, PPTX export hints, evidence metadata, and validation rules (DGE slide chrome: black title/subtitle, 30pt bold / 12pt regular Noto Sans, title clamped to two lines with ellipsis, subtitle anchored below that band, full-width body frame, top-right lockup 218x51px aligned with native cover lockup, section/subsection tracker pills #063360 on white text, structural theme blues biased to #0A4D73 instead of Office-like #215A9E)
