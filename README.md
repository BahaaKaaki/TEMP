...
- AI slide generation via PwC Shared Services (premium model: `bedrock.anthropic.claude-opus-4-8`)
...
- Theme and template system with CSS variables (Strategy& defaults include `--neutral-fill` and `--rose-fill` for chart and diagram fills)
- Client template profiles can switch generation away from the default Strategy& look; STC, PIF LDC, TDREDC, DGE, FYA, MoS, and SE ship as full profiles with semantic theme tokens, layout CSS variables, prompt-section overrides, bundled masters/assets, PPTX export hints, evidence metadata, and validation rules (DGE slide chrome: black title/subtitle with Noto Sans SemiBold 30px / Medium 12px, PPTX title chrome uses bold true on the SemiBold face, title clamped to two lines with ellipsis, subtitle anchored below that band, full-width body frame, top-right lockup 218x51px aligned with native cover lockup, section/subsection tracker pills **#203864** with white text, primary panels **#063360**, bundled lockup raster at `backend/assets/client-templates/dge/logo.png`; SE uses the custom SE font family, blue energy palette from brand guidelines, title 32px / subtitle 18px chrome, bottom-left logo chrome, and bundled master at `backend/assets/client-templates/se/default-master.pptx`; TDREDC reuses Fund fonts from the PIF family, uses a 13.333 x 7.5 in ExCom shell with top-left logo lockup, charcoal titles, green/gold agenda styling, and bundled master at `backend/assets/client-templates/tdredc/default-master.pptx`)
...
- **NEOM Authority** client profile (`neom`): NAFB5 master, slide-46 column patterns, ALL CAPS Arial 20pt bold title / 18pt regular subtitle, yellow `#EBC03F` headers, dark-badge contrast (`on-neutral-fill` on `#13100D`, not `on-accent`), bottom-left logo + chrome page number only, flat PPTX export (no NAFB5 section tree)
- **ECA CP** client profile (`eca`): white `1_Title Only - White` master from Template 2 (resized to 13.333x7.5in), Tahoma typography, teal `#41A4A5` titles, **no subtitle**, per-slide section tracker strip (number + label at y=4), orange `#D95022` / cyan `#53B0C5` / green `#7FBC80` accents, bundled master + logo at `backend/assets/client-templates/eca/`, `preserveTemplateChrome` on PPTX export
...
| Role | Model |
|---|---|
| Thinking (main generation) | `bedrock.anthropic.claude-opus-4-8` |
...
| Image | `openai.global.gpt-image-2` (global; longer runs). Proxy auto-falls back to `openai.gpt-image-1.5` / `openai.eu.gpt-image-1.5` on geography 401. **Visual Uplift** uses the same Image role model. Uplift is off by default; enable via debug Settings (`visualUpliftEnabled`; UI gate `VISUAL_UPLIFT_UI_ENABLED` in `featureFlags.js`). Chat/Quick Fix edits on image slides use split HTML vs image routing (`shouldRouteEditThroughImagePipeline`). Uplifted/image-content slides export to PPTX by embedding the raster image (not re-translating HTML). |
| Report | `bedrock.anthropic.claude-opus-4-8` |
| PPTX (export) | `bedrock.anthropic.claude-opus-4-8` |

Model assignments are server-controlled. The Settings modal shows which model is assigned to each role. The router uses GPT 5.5 with reasoning effort defaulting to high (including code fallback when router reasoning settings are missing); per-step evidence search defaults to GPT 5.4 mini, while premium slide generation, report compilation, and PPTX export default to **Claude Opus 4.8 on Bedrock** (`bedrock.anthropic.claude-opus-4-8`). The shared gateway rejects custom `temperature` values for `openai.gpt-5.5`; the client omits `temperature` for that model so LiteLLM uses the provider default. In debug mode, the Prompts section also exposes `promptOverrides` for individual prompt surfaces; empty overrides fall back to the code defaults.

## Pull requests

New pull requests are auto-assigned to **bahaa-kaaki_pwcit** (GitHub Action `.github/workflows/pr-auto-assign.yml`). Cursor agents also use `.cursor/rules/pr-assignee.mdc` and pass `--assignee bahaa-kaaki_pwcit` on `gh pr create`.
...
The TDREDC profile uses `20260628_TDREDC Template.pptx` as its bundled master. It reuses the PIF Fund font stack from `backend/assets/fonts/pif-fund/` and ships the full TDREDC lockup rasterized from template `image2.svg` (green-on-white variant) at `backend/assets/client-templates/tdredc/logo.png` (source SVG alongside). Generation follows the base Slide HTML Generator (minimal TDREDC contract: theme tokens, layout geometry, and chrome only — no separate vibe/writing/CSS prompt overrides). TDREDC standard slides map the 960 x 540 preview canvas to a 13.333 x 7.5 in PPTX deck; the logo lockup sits at `x=33 y=18 w=200 h=70` on standard body slides with the title band at `x=280 y=45 w=637 h=29`, an 885 x 418px body frame, blank footer/source text unless real source text exists, and a bottom-right gold page-number block. Workstream pillar colors from the native deck: Rise `#46121D`, NoRA `#9D7539`, Corporate Matters `#2C2C2C`, agenda legend `#29BA74`, detail panels `#00342B`.
