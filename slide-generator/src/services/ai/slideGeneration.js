...
import { PAIRED_DECK_JSON_SCHEMA, validatePairedDeck } from '../pairedPptx/index.js';
...
// ============================================
// PAIRED PPTX CO-GENERATION (Direction B)
// ============================================
//
// When enabled, a single LLM call returns { slides: [{ html, customCss,
// pptxOps }] } where pptxOps is a structured array of editable-PowerPoint
// instructions. The export pipeline consumes pptxOps directly instead of
// re-inferring PPTX geometry from the rendered HTML. See:
//   - slide-generator/src/services/pairedPptx/schema.js (contract)
//   - slide-generator/src/services/pairedPptx/emitter.js (ops -> pptxgenjs)
//   - slide-generator/src/guides/freestyle-pptx-emit.md (LLM instructions)
...
export async function generateSlidesPaired(prompt, settings, slideCount, existingSlides, contextInfo, opts = {}) {
  ...
  const pairedSettings = {
    ...settings,
    pairedPptxMode: true,
    _responseJsonSchema: PAIRED_DECK_JSON_SCHEMA,
  };

  const activeSystemPrompt = buildFreestyleSystemPrompt(pairedSettings);
  ...
  if (isAutoCount) {
    countInstruction = 'Decide the appropriate number of slides (1-5) based on the content needs.';
  } else {
    const totalSlides = existingSlides.length + targetCount;
    countInstruction = `Generate exactly ${targetCount} slide(s). These will be slides ${startingSlideNum} to ${startingSlideNum + targetCount - 1} of the deck. Number the footer as "X / ${totalSlides}" starting from ${startingSlideNum}.`;
  }
...
`;
  ...
  const layoutHint = routerLayoutGuidance
    ? `\nLAYOUT HINT (from narrative planner): "${routerLayoutGuidance}" - consider this, but choose whatever layout best serves the content.`
    : '';

  const userPrompt = `Create professional presentation slide(s) about: "${prompt}"
...
PAIRED OUTPUT (MANDATORY):
- Return a JSON object matching the PairedDeckOutput schema: { "slides": [PairedSlideOutput, ...] }.
- Each slide has "html" (the preview HTML), "customCss" (scoped CSS string, or empty), and "pptxOps" (array of editable-PowerPoint instructions).
- See the Paired PPTX emit guide section of your system prompt for every op type, rule, and pairing convention.
- Every positioned text/shape/image/table/chart in each slide's HTML must have both a data-pptx-id attribute AND a matching entry in pptxOps with the same id.
- Do NOT wrap the JSON in markdown fences. Do NOT add explanation text. Return the JSON object only.

CONTENT FIDELITY:
- TOPIC PROMPT -> you generate the content. Be professional, specific, data-rich.
...
- Sources go ONLY in footer, never inside frame.`;

  console.groupCollapsed(
    '[Paired] LLM prompt (system: %d chars, user: %d chars)',
  ...
  let raw;
  try {
    raw = await callWithModelFallback(pairedSettings, activeSystemPrompt, userPrompt, opts);
  } catch (err) {
    debugLog(LogLevel.ERROR, 'generateSlidesPaired', 'API call failed', { msg: err.message });
    throw err;
  }

  let parsed = null;
  try {
    const objText = extractFirstJsonObject(raw) || stripCodeFences(raw);
    if (!objText) throw new Error('No JSON object found in response');
    parsed = JSON.parse(objText);
  } catch (parseErr) {
    console.warn('[Paired] Failed to parse JSON on first pass:', parseErr.message);
    const fixedRaw = await callWithModelFallback(
      pairedSettings,
      activeSystemPrompt,
      `${userPrompt}\n\nYour previous response was not valid JSON (parse error: ${parseErr.message}). Re-emit the ENTIRE response as a single JSON object matching the PairedDeckOutput schema. No markdown. No commentary.`
    );
    try {
      const fixedText = extractFirstJsonObject(fixedRaw) || stripCodeFences(fixedRaw);
      parsed = JSON.parse(fixedText || '{}');
    } catch (retryErr) {
      audit('paired.parse_failed', { error: retryErr.message, preview: fixedRaw?.slice(0, 200) });
      throw new Error(`Paired JSON parse failed on retry: ${retryErr.message}`);
    }
  }

  const deckReport = validatePairedDeck(parsed);
  if (!deckReport.ok) {
    console.warn(
      '[Paired] Schema validation found %d error(s), requesting correction',
      deckReport.errors.length,
      deckReport.errors.slice(0, 10)
    );
    const correctionPrompt = `Your previous response did not match the PairedDeckOutput schema. Errors:
${deckReport.errors.slice(0, 15).map((e) => `- ${e.path}: ${e.message}`).join('\n')}

Re-emit the entire response as a single valid JSON object that passes the schema. Keep the slide content but fix the listed issues. No markdown. No commentary.

Original response (for reference):
${raw.slice(0, 4000)}`;

    try {
      const correctedRaw = await callWithModelFallback(pairedSettings, activeSystemPrompt, correctionPrompt);
      const correctedText = extractFirstJsonObject(correctedRaw) || stripCodeFences(correctedRaw);
      const correctedParsed = JSON.parse(correctedText || '{}');
      const correctedReport = validatePairedDeck(correctedParsed);
      if (correctedReport.ok) {
        console.log('%c[Paired] Correction pass succeeded', 'color:#059669;font-weight:bold');
        parsed = correctedParsed;
      } else {
        console.warn('[Paired] Correction pass still invalid (%d errors), giving up', correctedReport.errors.length);
        throw new Error(`Paired schema invalid after correction: ${correctedReport.errors[0]?.message || 'unknown'}`);
      }
    } catch (corrErr) {
      audit('paired.validate_failed', { errors: deckReport.errors.slice(0, 10), correction: corrErr.message });
      throw new Error(`Paired schema validation failed: ${corrErr.message}`);
    }
  }

  const slideObjs = parsed.slides.map((pairedSlide, index) => {
    let slideHtml = ensureSlideStructure(pairedSlide.html);
    const title = extractTitleFromHTML(slideHtml) || `Slide ${index + 1}`;
    const type = detectSlideType(slideHtml);
    return {
      html: slideHtml,
      title,
      type,
      summary: generateSlideSummary(slideHtml, type, title),
      ...(pairedSlide.customCss ? { customCSS: pairedSlide.customCss } : {}),
      pptxOps: pairedSlide.pptxOps,
      _paired: true,
    };
  });

  console.log(
    '%c[Paired] Generated %d slide(s) with %d total pptxOps',
    'color:#059669;font-weight:bold',
    slideObjs.length,
    slideObjs.reduce((s, x) => s + (x.pptxOps?.length || 0), 0)
  );
...
export async function generateSlides(prompt, settings, slideCount = 3, existingSlides = [], templateId = null, customTemplate = null, contextInfo = null, opts = {}) {
  ...
  // Paired PPTX co-generation path. Gated by settings.pairedPptxMode and only
  // available for freestyle generation (no template / no custom system prompt
  // overrides). On validation failure the function falls back to the legacy
  // HTML-only path silently, so callers never need to branch on the mode.
  // All three provider families now carry structured-output support:
  //   - OpenAI: response_format / Responses text.format json_schema
  //   - Gemini: generationConfig.responseSchema + responseMimeType json
  //   - Anthropic: forced tool_use whose input_schema is the PAIRED schema
  if (
    settings.pairedPptxMode &&
    isFreestyle &&
    !customTemplate &&
    !templateId &&
    ...
    try {
      const paired = await generateSlidesPaired(
        prompt,
        settings,
        slideCount,
        existingSlides,
      ...
      if (paired && paired.length > 0) return paired;
      console.warn('[Paired] generateSlidesPaired returned empty, falling back to legacy HTML path');
    } catch (pairedErr) {
      console.warn(
        '[Paired] generateSlidesPaired failed, falling back to legacy HTML path:',
        pairedErr.message?.slice(0, 200)
      );
    }
  }
