...
import { isCoverSlideHtml } from '../utils/slideChromeUtils.js';

// ── Constants ────────────────────────────────────────────────────────────────

const MAX_RETRIES = 3; // 4 total attempts per slide
const DEFAULT_PPTX_MODEL = 'pwc:bedrock.anthropic.claude-opus-4-8';
...
async function writeSanitizedPptxFile(pptx, filename, label = 'direct export', profile = null) {
  const buf = await pptx.write({ outputType: 'arraybuffer' });
  try {
    const sanitized = await sanitizePptxBufferForPowerPoint(buf, label, { profile });
...
function getProfilePptxTypographyGuidance(profile) {
  ...
  if (profile?.id === 'tdredc') {
    return '- TDREDC typography: use Fund Light for titles/body and Fund Regular only for page numbers or limited emphasis. Content title is 16pt in PPTX, dense agenda/detail text may be 8-10pt, and footer/source/page chrome may be 7-8pt. There is no broad subtitle band on standard TDREDC body slides.\n';
  }
  if (profile?.id === 'npc') {
    return '- NPC typography: use Calibri for titles, body, footnotes, and page numbers; Calibri Light only for large cover display headings. Content title is 28pt bold Qatar maroon (#89143C); an optional 18pt bold black kicker line may sit directly under the title. Body is ~14-15pt black, footnotes/page chrome 7-8pt.\n';
  }
  if (profile?.id === 'dge') {
...
ACTIVE CLIENT PROFILE OVERRIDE -- TDREDC:
- Use the verified TDREDC ExCom master shell, not generic Strategy& geometry.
- TDREDC PPTX canvas is 13.333 x 7.5 inches. HTML 960 x 540 px maps with px * 13.333 / 960.
- TDREDC colors: title charcoal 595959, dark green 00332A, green 005C4D, gold/brown C4995B/9D7539/654B25, mint 29BA74, white/light-neutral surfaces, gray 7F7F7F.
- Workstream pillar headers: Rise 46121D, NoRA 9D7539, Corporate Matters 2C2C2C (white label text on each). Agenda legend labels 29BA74. Dense update panels 00342B.
- If you define a c palette, use: main:'595959', secondary:'00332A', accent:'005C4D', gold:'C4995B', brown:'654B25', mint:'29BA74', rise:'46121D', nora:'9D7539', corporate:'2C2C2C', panelDark:'00342B', surface:'FFFCF2', surfaceAlt:'F2F2F2', border:'DCC29C', meta:'7F7F7F'.
- Use Fund Light for every normal text box; use Fund Regular only for page numbers or limited emphasis. Do not use Arial, Georgia, Calibri, Aptos, STC Forward, or emoji.
- Follow the TDREDC master geometry: content title x=1.167 y=0.469 w=11.198 h=0.302, body x=0.458 y=0.833 w=12.281 h=4.361, footer/source x=0.458 y=5.271, page block x=12.433 y=5.354. Cover slides only: logo x=0.458 y=0.250 w=2.778 h=0.972 with title below.
- Do not add a broad subtitle under the title. If a lens is needed, use a compact top-right label or in-exhibit label.
- Keep footer/source text blank unless the user explicitly provides it. Do not surface review notes, scratch pages, or hidden think-cell artifacts.
`;
  }
  if (profile?.id === 'npc') {
    return `

ACTIVE CLIENT PROFILE OVERRIDE -- NPC (National Planning Council, State of Qatar):
- Use the verified NPC master shell, not generic Strategy& geometry.
- NPC PPTX canvas is 13.333 x 7.5 inches. HTML 960 x 540 px maps with px * 13.333 / 960.
- NPC colors: Qatar maroon 89143C (content titles, page numbers, primary brand), cover panel maroon 8A1538, deep teal-blue 0F4260, green 129B82, blue 4194B3, gold A29160, sand/cream ECE8DF, pale yellow FDF49C, black 000000 body text, gray 595959 muted, white surfaces.
- If you define a c palette, use: main:'89143C', secondary:'000000', accent:'0F4260', maroon:'8A1538', teal:'0F4260', green:'129B82', blue:'4194B3', gold:'A29160', sand:'ECE8DF', pale:'FDF49C', surface:'FFFFFF', surfaceAlt:'ECE8DF', border:'D9D2C4', meta:'595959'.
- Use Calibri for every normal text box; Calibri Light only for large cover display headings. Do not use Aptos, Georgia, Times, Fund fonts, STC Forward, or emoji.
- Follow the NPC master geometry: content title x=0.292 y=0.431 w=12.625 h=0.833 (28pt bold maroon 89143C), optional compact bold black kicker directly below, body x=0.292 y=1.847 w=12.625 h=4.792, footnotes/source x=0.292 y=6.944 w=11.0, page-number block x=12.306 y=6.917 (7pt maroon inside the gold-outlined diamond). Cover slides only: NPC + Qatar-emblem lockup x=9.681 y=0.292 w=3.236 h=0.944 top-right, over a lower maroon 8A1538 panel with white ~45pt title.
- Do not add a broad subtitle band. An optional bold kicker line under the title is allowed; keep it compact.
- Keep footer/source text blank unless the user explicitly provides it. Do not surface embedded think-cell artifacts ("think-cell data - do not delete"), review notes, or scratch pages.
`;
  }
  if (profile?.id === 'dge') {
    return `
...
- Brand palette taken from the native DGE band gradient + chrome shapes: primary brand blue **#005393** (the dominant visible blue on header bands, scope rows, panels), deepest navy **#00437B** (covers, darkest fills), bright top-of-band **#2A70AD** (highlights), secondary chrome navy **#203864** (small structural shapes only — not as a dominant fill), light panel **#7DA1C4**, very light **#A6CAEC**, pale surfaces #E7E6E6 / #F2F2F2, white.
- **Text rule (strict):** all body, title, subtitle, label, KPI, and caption text uses **black #000000 on light surfaces** and **white #FFFFFF on dark/blue fills** — NEVER blue text. The blue palette is for fills, bands, panels, rules, and chrome only. Muted/secondary text uses #4A5568 grey, not blue. Do NOT use #063360 — it is not in the native palette.
- If you define a c palette, favor: main:'000000', onDark:'FFFFFF', fill:'005393', chrome:'203864', deep:'00437B', highlight:'2A70AD', accent:'A6CAEC', surface:'F2F2F2', border:'7DA1C4', meta:'4A5568'.
...
function buildProfileReferenceExampleCode(profile, fallbackCode) {
  ...
  const subtitle = positions.subtitle;
...
const PROFILE_PPTX_FONT_FACE_VARIANTS = {
  ...
  tdredc: ['Fund Light', 'Fund Regular', 'Fund Med', 'Fund SemBd'],
  npc: ['Calibri', 'Calibri Light'],
...
function enforcePptxColorsForProfile(codeString, profile) {
  ...
  if (profile?.id === 'pif') {
    ...
    // #063360, #0E2841, #32516E and #0A4D73 are remapped to the visible blue palette.
    // Text colors stay black or white — native DGE never writes body/heading text in
    // brand blue, so dark-text tokens (#111111/#222222/#A32020/#8E1E1E/#FF375E) all
    // map to black, not to a blue.
    replacements = new Map([
      ['111111', '000000'],
      ['222222', '000000'],
      ['A32020', '000000'],
...
INLINE SVG ICONS: Only the SVG path/glyph itself is rasterized from the rendered HTML and overlaid as a small transparent PNG after your code runs. Do NOT try to redraw those SVG path glyphs as emoji, placeholder text, or embedded base64 images. You ARE still responsible for rendering everything around the glyph from the CSS: the surrounding card, the icon chip background (filled circle / rounded square / border), borders, and all text. Draw the chip as a native PptxGenJS ellipse/roundRect using the fill, border, and size from the CSS — the rasterized icon glyph will land cleanly on top of it.
...
async function callClaudeAPI(settings, credentials, systemPrompt, userPrompt) {
  ...
  const noTemp = /claude-opus-4-[78]|claude-sonnet-4-6/i.test(mdl);
...
function buildProfilePositionBlock(profile) {
  ...
  if (standard.subtitle) {
    positions.subtitle = pxRectToInches(standard.subtitle, layout.canvas);
  }
...
function shouldSuppressStandardSubtitle(profile) {
  return ['pif', 'tdredc', 'dge'].includes(profile?.id);
}
...
function normalizeProfileTextBoxOptions(objects, profile) {
  if (!['pif', 'tdredc'].includes(profile?.id) || !Array.isArray(objects)) return;
...
export async function exportToPPTX(slides, filename = 'presentation.pptx', settings = null, onProgress = null) {
  ...
  const coverSlideNumbers = slides
    .map((s, i) => (isCoverSlideHtml(s?.html) ? i + 1 : null))
    .filter(n => n != null);
  ...
  if (templateData?.data && !shouldUseDirectProfileChromeExport(activeProfile)) {
    try {
      ...
      const merged = useControlledProfileChrome
        ? await applyProfileChromeToGenerated(buf, chrome, { profile: activeProfile, templateData: templateData.data, coverSlideNumbers })
        : await applyTemplateToGenerated(buf, templateData.data, chrome, {
          preserveTemplateChrome: activeProfile.id === 'strategy' || activeProfile.id === 'adsc' || activeProfile.id === 'remat' || activeProfile.id === 'eca',
      ...
      await writeSanitizedPptxFile(pptx, filename, 'template merge fallback', activeProfile);
    }
  } else {
    await writeSanitizedPptxFile(pptx, filename, shouldUseDirectProfileChromeExport(activeProfile) ? `${activeProfile.id} direct profile export` : 'direct export', activeProfile);
  }
...
export async function exportSingleSlideToPPTX(slide, slideNumber, totalSlides, filename, settings = null, onProgress = null) {
  ...
  const coverSlideNumbers = isCoverSlideHtml(slide?.html) ? [1] : [];
  ...
  if (templateData?.data && !shouldUseDirectProfileChromeExport(activeProfile)) {
    try {
      ...
      downloadArrayBuffer(merged, filename);
    } catch (e) { await writeSanitizedPptxFile(pptx, filename, 'single-slide template fallback', activeProfile); }
    } else {
      await writeSanitizedPptxFile(pptx, filename, shouldUseDirectProfileChromeExport(activeProfile) ? `${activeProfile.id} single-slide direct profile export` : 'single-slide direct export', activeProfile);
    }
