...
function getProfilePptxTypographyGuidance(profile) {
  ...
  if (profile?.id === 'dge') {
    return '- DGE typography: Noto Sans SemiBold for standard slide titles, Noto Sans Medium for subtitles, Noto Sans Regular for body and footer. Slide chrome title/subtitle text: black #000000, ~30pt title, ~12pt subtitle (use those face names in PPTX, not generic bold flags on Noto Sans). Body/footer use the profile greys and brand blues only inside content—not for the top title bands.\n';
  }
