...
async function loadProfileLogo(profile) {
  const logoPos = profile?.chrome?.logoScope === 'cover-only'
    ? profile?.chrome?.coverPositions?.logo
    : (profile?.chrome?.coverPositions?.logo || profile?.chrome?.positions?.logo);
...
function stripDanglingTagMetadata(xml) {
  return String(xml || '')
    .replace(/<p:custDataLst>[\s\S]*?<\/p:custDataLst>/g, '')
    .replace(/<p:tags\b[^>]*\/>/g, '')
    .replace(/<p:extLst>[\s\S]*?think-cell[\s\S]*?<\/p:extLst>/gi, '');
}

async function generatePptxArrayBuffer(zip, options = {}) {
  const profileId = options.profile?.id || options.profileId || null;
  if (profileId === 'eca') {
...
async function sanitizePptxZipForPowerPoint(zip, label = 'presentation', options = {}) {
  const profileId = options.profile?.id || options.profileId || null;
  const stripTagMetadata = profileId === 'eca';
  ...
  for (const xmlPath of pptXmlFiles) {
    ...
    if (stripTagMetadata) {
      fixed = stripDanglingTagMetadata(fixed);
    }
...
export async function sanitizePptxBufferForPowerPoint(buffer, label = 'presentation', options = {}) {
  const zip = await JSZip.loadAsync(buffer);
  await sanitizePptxZipForPowerPoint(zip, label, options);
  return generatePptxArrayBuffer(zip, options);
}

function shouldInjectProfileLogo(profile, slideNum, coverSlideNumbers = []) {
  if (profile?.chrome?.logoScope === 'cover-only') {
    return coverSlideNumbers.includes(slideNum);
  }
  return true;
}
...
export async function applyProfileChromeToGenerated(generatedBuf, chrome = null, options = {}) {
  ...
  for (const genSlidePath of genSlideFiles) {
    ...
    } else if (hasLogo && shouldInjectProfileLogo(profile, slideNum, coverSlideNumbers)) {
      slideXml = injectLogoPic(slideXml, chrome.logo, LOGO_RID);
    }
    genZip.file(genSlidePath, slideXml);

    if (!hasLogo) continue;
    if (!shouldInjectProfileLogo(profile, slideNum, coverSlideNumbers)) continue;
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  await sanitizePptxZipForPowerPoint(genZip, 'profile chrome export', { profile });
  const result = await generatePptxArrayBuffer(genZip, { profile });
...
async function pickBestLayout(tplZip, profile = null) {
  ...
  if (profile?.id === 'eca') {
    for (const path of layoutFiles) {
      const xml = await tplZip.files[path].async('string');
      const name = xml.match(/<p:cSld\s+name="([^"]+)"/)?.[1] || '';
      if (/1_title only\s*-\s*white/i.test(name)) {
        const idx = parseInt(path.match(/slideLayout(\d+)/)[1], 10);
        console.log('[PPTX Template] ECA CP: using white content layout "%s" (%d)', name, idx);
        return idx;
      }
    }
  }
...
export async function applyTemplateToGenerated(generatedBuf, templateBuf, chrome = null, options = {}) {
  ...
  for (let i = 0; i < slideCount; i++) {
    ...
    if (genZip.files[genSlideRelsPath]) {
      ...
      if (hasLogo && shouldInjectProfileLogo(profile, slideNum, coverSlideNumbers)) {
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  await sanitizePptxZipForPowerPoint(tplZip, 'template merge export', { profile });
  const result = await generatePptxArrayBuffer(tplZip, { profile });
