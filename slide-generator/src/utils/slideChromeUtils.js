...
function stripTdredcTopLogoDuplicates(html = '') {
  return html
    .replace(
      /(<div[^>]*class="[^"]*\bslide\b[^"]*"[^>]*>)\s*<img\b(?![^>]*\bclient-chrome-tdredc-logo\b)[^>]*>/gi,
      '$1',
    )
    .replace(
      /(<h1[^>]*class="[^"]*\btitle\b[^"]*"[^>]*>)([\s\S]*?)(<\/h1>)/gi,
    ...
    .replace(
      /<img\b(?![^>]*\bclient-chrome-tdredc-logo\b)[^>]*\b(?:\blogo\b|\bbrand\b|\btdredc\b|\bpif\b|lockup|wordmark)\b[^>]*>/gi,
      '',
    );
}
...
/**
 * Shared cover-slide predicate used by both preview chrome injection and PPTX
 * export so they agree on which slides are covers (drives cover-only logo scope).
 */
export function isCoverSlideHtml(html) {
  return /\b(cover-slide|cover-branding|master-cover)\b/i.test(html || '');
}
...
export function injectClientProfileChrome(html, profile, logoUrl, logoIconUrl = null) {
  ...
  if (profile.id === 'tdredc') {
    cleanedHtml = stripTdredcTopLogoDuplicates(cleanedHtml);
  }
  ...
  const isCoverSlide = isCoverSlideHtml(cleanedHtml);
  if (isSpecialMaster) {
    ...
    // Cover-only profiles (TDREDC ExCom, NPC) still receive the bundled lockup on
    // their covers; body/special-master slides do not.
    if (!(profile.chrome?.logoScope === 'cover-only' && isCoverSlide)) {
      return withProfile;
    }
  }

  if (profile.chrome?.injectPreviewLogo === false) return withProfile;
  // Cover-only profiles (e.g. TDREDC ExCom, NPC) have no header logo on body
  // slides — only covers carry the bundled lockup.
  if (profile.chrome?.logoScope === 'cover-only' && !isCoverSlide) {
    return withProfile;
  }
...
function hasBundledChrome(profile) {
  return (profile?.chrome?.positions?.logo || profile?.chrome?.coverPositions?.logo)
