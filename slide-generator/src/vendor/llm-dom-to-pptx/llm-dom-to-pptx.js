 ...
 *
 * Vendored from npm package llm-dom-to-pptx@1.2.7 (MIT).
 *
 * Local patches vs upstream:
 *   1. ESM import of PptxGenJS (assigned to window.PptxGenJS) replaces the
 *      requirement for <script src="pptxgen.bundle.js"> CDN tags.
 *   2. Default CONFIG switched to widescreen 13.333 x 7.5 in to match
 *      slide-generator conventions (upstream was 10 x 5.625).
 *   3. Runtime layout hardcoding replaced with defineLayout("CUSTOM_WIDE").
 *   4. An ESM `exportLlmDomToPptx` wrapper is appended at the bottom.
 */
import PptxGenJSModule from "pptxgenjs";
if (typeof window !== 'undefined' && !window.PptxGenJS) {
    window.PptxGenJS = PptxGenJSModule;
}
...
(function (root) {
    ...
    // Map CSS transform: rotate(...) and writing-mode onto pptxgenjs rotation.
    // Returns { vert: 'vert' | 'vert270' | 'eaVert' | 'mongolianVert' } for 90-degree
    // increments (preferred, keeps the text box axis-aligned in PowerPoint), or
    // { rotate: <deg 0..359> } for arbitrary angles, or null when no rotation applies.
    function extractRotationFromStyle(style) {
        if (style) {
            const wm = style.writingMode;
            if (wm === 'vertical-rl' || wm === 'sideways-rl') return { vert: 'eaVert' };
            if (wm === 'vertical-lr' || wm === 'sideways-lr') return { vert: 'mongolianVert' };
        }
        const tr = style && style.transform;
        ...
        if (deg === 90) return { vert: 'vert' };
        if (deg === 270) return { vert: 'vert270' };
        return { rotate: deg };
    }
    ...
    class SlideExporter {
        ...
        renderText(node, style, rect, opacity = 1.0) {
            ...
            if (tW > 0 && tH > 0) {
                ...
                // Map writing-mode / transform: rotate(...) onto pptxgenjs options.
                const rotInfo = extractRotationFromStyle(style);
                if (rotInfo) {
                    if (rotInfo.vert) textOpts.vert = rotInfo.vert;
                    else if (typeof rotInfo.rotate === 'number') textOpts.rotate = rotInfo.rotate;
                }
        ...
        collectTextRuns(node, parentStyle, opacity = 1.0) {
            ...
            node.childNodes.forEach(child => {
                if (child.nodeType === Node.TEXT_NODE) {
                    ...
                    // CSS text-transform is a visual effect only; textContent keeps the raw
                    // string. Mirror it onto the extracted run so PPTX reflects what the
                    // browser shows (LOCAL PATCH #5).
                    const tt = s.textTransform;
                    if (tt === 'uppercase') clean = clean.toUpperCase();
                    else if (tt === 'lowercase') clean = clean.toLowerCase();
                    else if (tt === 'capitalize') clean = clean.replace(/(^|\s)(\S)/g, (_, p1, p2) => p1 + p2.toUpperCase());
                    ...
                    if (s.textDecoration && s.textDecoration.includes('line-through')) run.options.strike = 'sngStrike';

                    // letter-spacing -> charSpacing. CSS is in px, pptxgenjs charSpacing is in
                    // points (1 px = 0.75 pt at 96 DPI). Skip values under ~0.1 pt.
                    const lsPx = parseFloat(s.letterSpacing);
                    if (!isNaN(lsPx) && Math.abs(lsPx) > 0) {
                        const ls = Math.round(lsPx * 0.75 * 10) / 10;
                        if (Math.abs(ls) >= 0.1) run.options.charSpacing = ls;
                    }
                    ...
                    if (child.tagName === 'BR') {
                        ...
                        const childRuns = this.collectTextRuns(child, s, childOpacity);

                        // <sup> / <sub> -> superscript / subscript on every child run.
                        if (child.tagName === 'SUP' || child.tagName === 'SUB') {
                            const key = child.tagName === 'SUP' ? 'superscript' : 'subscript';
                            childRuns.forEach((r) => { r.options = Object.assign({}, r.options, { [key]: true }); });
                        }

                        // <a href> -> pptxgenjs hyperlink on every child run.
                        if (child.tagName === 'A') {
                            const href = child.getAttribute('href');
                            if (href && href !== '#' && !href.startsWith('javascript:')) {
                                childRuns.forEach((r) => {
                                    r.options = Object.assign({}, r.options, {
                                        hyperlink: { url: href, tooltip: child.getAttribute('title') || href },
                                    });
                                });
                            }
                        }
    ...
    async function exportToPPTX(elementOrId, options = {}) {
        ...
        pres.defineLayout({ name: 'CUSTOM_WIDE', width: CONFIG.PPT_WIDTH_IN, height: CONFIG.PPT_HEIGHT_IN });
...
// ESM wrapper appended by slide-generator vendor patch.
// Calls the upstream IIFE-attached global after ensuring PptxGenJS is available.
export function exportLlmDomToPptx(selectorOrEl, options = {}) {
    if (typeof window === 'undefined' || !window.LLMDomToPptx || typeof window.LLMDomToPptx.export !== 'function') {
        throw new Error('[llm-dom-to-pptx] IIFE failed to initialize LLMDomToPptx on window');
    }
    return window.LLMDomToPptx.export(selectorOrEl, options);
}
